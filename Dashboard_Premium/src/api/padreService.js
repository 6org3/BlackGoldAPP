...
// Supabase no lanza ante un fallo de red o de RLS: devuelve { data: null, error }.
// Sin este chequeo el portal del padre trataba el fallo como "no hay datos" y se
// quedaba en CARGANDO… para siempre, sin mensaje ni reintento.
const oFalla = (error, que) => {
  if (error) throw new Error(`No se pudo cargar ${que}: ${error.message || error}`);
};
...
export const fetchPadreData = async (padreId) => {
  ...
  const { data: vinculos, error: eVinculos } = await supabase
    ...
    .select('atleta_id')
    .eq('padre_id', padreId);
  oFalla(eVinculos, 'la lista de hijos');

  if (!vinculos || vinculos.length === 0) return { hijos: [], sesiones: [], anuncios: [] };

  const idsHijos = vinculos.map(v => v.atleta_id);

  // 2. Traer datos de los atletas (hijos)
  const { data: atletas, error: eAtletas } = await supabase
    .from('atletas')
    .select(`
      *,
    ...
    `)
    .in('id', idsHijos);
  oFalla(eAtletas, 'la ficha de los hijos');

  // Traer evaluaciones para construir radar correctamente
  const { data: evaluaciones, error: eEvals } = await supabase
    .from('evaluaciones_pruebas')
    .select('*')
    .in('atleta_id', idsHijos)
    .order('created_at', { ascending: false });
  oFalla(eEvals, 'las evaluaciones');
  ...
  const { data: sesiones, error: eSesiones } = await supabase
