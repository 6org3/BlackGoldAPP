    ...
    <div className="space-y-6 animate-in slide-in-from-right-4">
      {successMsg ? (
        <div className="flex flex-col items-center justify-center py-10 space-y-4">
          <div className="w-16 h-16 bg-success/20 text-success-soft rounded-full flex items-center justify-center"><Check size={32} /></div>
          <p className="text-success-soft font-bold uppercase tracking-widest">{successMsg}</p>
        </div>
      ) : (
        <>
          <div className="flex justify-between items-center mb-6">
            <button onClick={() => setStep(4)} className="text-xs text-fg-secondary hover:text-white flex items-center">
                <ChevronLeft size={14} className="mr-1"/> Volver
            </button>
            <div className="text-right">
                <p className="text-2xs text-fg-secondary font-bold uppercase tracking-widest">Evaluando a</p>
                <p className="text-xl font-black text-brand">{atletaEvaluando.nombre}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {['esfuerzo', 'actitud', 'foco', 'trabajo_equipo'].map(metric => (
              <div key={metric} role="group" aria-label={METRIC_LABELS[metric]}
                className="flex flex-col space-y-2 bg-white/5 p-3 rounded-control border border-white/10 items-center hover:bg-white/10 transition-colors">
                ...
                <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map(star => (
                    <button key={star} type="button" onClick={() => handleRatingChange(metric, star)}
                      ...
                      <Star size={28} fill={star <= ratings[metric] ? COLORS.gold[500] : 'transparent'} color={star <= ratings[metric] ? COLORS.gold[500] : COLORS.fg.faint} />
                    </button>
                  ))}
                </div>
              </div>
          ...
          <div className="pt-4 border-t border-white/10">
            <p className="text-2xs text-brand font-bold uppercase tracking-widest mb-3 flex items-center"><Target size={12} className="mr-1" /> Insignias Automáticas (5 Estrellas)</p>
