  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col space-y-4 max-w-md mx-auto">
        <label className="text-xs text-[#FFD700] uppercase font-bold tracking-widest text-center">Plantilla a Entrenar Hoy</label>
        <p className="text-gray-400 text-xs text-center mb-2">La plantilla define el objetivo de la clase; los asistentes reciben XP al finalizarla.</p>
              ...
              {grupo.items.map(p => {
                ...
                return (
                  <button key={p.id} onClick={() => setPlantillaSeleccionada(p)} aria-pressed={activa}
                    className={`py-3 px-4 rounded-xl text-sm font-bold uppercase tracking-wide border text-left transition-all ${activa ? 'bg-[#FFD700]/10 border-[#FFD700] text-[#FFD700]' : 'bg-white/5 border-white/10 text-white hover:bg-white/10'}`}>
                    <span className="block">{p.titulo}</span>
                    {p.sub_pilar && (
                      <span className="block text-[9px] font-bold normal-case tracking-normal text-gray-500 mt-0.5">
