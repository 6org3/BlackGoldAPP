    ...
    <div>
      ...
      <div style={{ position: 'relative', height: 180, border: `1px solid ${BORDER.neutralSoft}`, clipPath: cut(12), background: 'repeating-linear-gradient(45deg, rgba(255,255,255,.035) 0 10px, rgba(13,13,16,.9) 10px 20px)', display: 'grid', placeItems: 'center', marginBottom: 8 }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <MicroLabel color={C.text3} size={11} tracking=".08em">VIDEO · TÉCNICA DE LA MISIÓN</MicroLabel>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: C.card, border: `1px solid rgba(255,215,0,.2)`, clipPath: cut(10), padding: '12px 14px', marginBottom: 14 }}>
        <div style={{ flex: 1 }}>
          <MicroLabel color={C.text3} size={11} tracking=".06em">TU PROGRESO</MicroLabel>
      ...
      {/* Pasos */}
      <MicroLabel color={C.text3} size={11} style={{ margin: '0 0 8px' }}>PASOS</MicroLabel>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginBottom: 16 }}>
        {ctx.pasos.map((p, i) => (
          <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'baseline' }}>
            <span style={{ fontFamily: PIXEL, fontSize: 11, color: C.gold, flex: 'none' }}>{i + 1}</span>
      ...
      {ctx.enviada && (
        <div style={{ textAlign: 'center', padding: 13, background: 'rgba(168,85,247,.1)', border: `1px solid rgba(168,85,247,.4)`, color: C.ai, clipPath: cut(10), fontFamily: PIXEL, fontSize: 11, marginBottom: 16, animation: 'bg-pop .4s ease-out' }}>
          ✓ ENVIADA · EL COACH LA REVISA
        </div>
      )}

      {/* Mini-quiz */}
      <MicroLabel color={C.ai} size={11} style={{ margin: '0 0 10px' }}>MINI-QUIZ · 3 PREGUNTAS</MicroLabel>
          ...
          <div key={qi} style={{ background: C.card, border: `1px solid ${BORDER.neutral}`, clipPath: cut(10), padding: '13px 14px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {q.opts.map((o, oi) => (
                ...
                >
                  <span style={{ fontFamily: PIXEL, fontSize: 11, color: o.selected ? C.gold : C.text4, flex: 'none' }}>{o.selected ? '■' : '□'}</span>
