    ...
    <div>
      ...
      <div style={{ background: C.card, border: '1px solid rgba(52,211,153,.25)', clipPath: cut(14), padding: 16, marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 10 }}>
          <div>
            <MicroLabel color={C.text3} size={11} tracking=".06em">ASISTENCIA MEDIA · 30 DÍAS</MicroLabel>
            <p style={{ margin: '5px 0 0', fontFamily: PIXEL, fontSize: 30, color: C.ok, lineHeight: 1 }}>{ctx.mediaLabel}</p>
          </div>
          <span style={{ flex: 'none', fontFamily: PIXEL, fontSize: 11, color: C.ok, border: '1px solid rgba(52,211,153,.35)', padding: '6px 9px' }}>{ctx.mediaTrend}</span>
        </div>
      ...
      {/* Por categoría */}
      <MicroLabel color={C.text3} size={11} style={{ margin: '0 0 8px' }}>POR CATEGORÍA</MicroLabel>
        ...
        {ctx.catRows.map((cr, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, background: C.card, border: `1px solid ${cr.border}`, clipPath: cut(8), padding: '10px 12px' }}>
            <div style={{ width: 86, flex: 'none' }}>
              <MicroLabel color={cr.color} size={11} tracking="normal">{cr.label}</MicroLabel>
              <p style={{ margin: '2px 0 0', fontSize: 11, color: C.text3 }}>{cr.sub}</p>
            </div>
            <div style={{ flex: 1, display: 'flex', gap: 2 }}>
              {cr.cells.map((c, j) => <span key={j} style={{ flex: 1, height: 9, background: c }} />)}
            </div>
            <span style={{ flex: 'none', width: 34, textAlign: 'right', fontFamily: PIXEL, fontSize: 11, color: cr.color }}>{cr.pctLabel}</span>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        <MicroLabel color={C.text3} size={11}>OCUPACIÓN · CANCHA CENTRAL</MicroLabel>
        <MicroLabel color={C.text4} size={11} tracking="normal">TOCA UNA CELDA</MicroLabel>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'rgba(255,215,0,.06)', border: '1px solid rgba(255,215,0,.28)', clipPath: cut(10), padding: '12px 14px' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <MicroLabel color={C.gold} size={11} tracking=".06em">{ctx.heatTitle}</MicroLabel>
