...
export default function PanelResumen({ ctx }) {
  return (
    <div>
      ...
      <MicroLabel color={C.text3} size={11} style={{ margin: '0 0 8px' }}>ALERTAS · REQUIEREN ACCIÓN</MicroLabel>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 18 }}>
        {ctx.alertas.map((al, i) => (
          <button key={i} type="button" onClick={al.onGo} style={{ display: 'flex', alignItems: 'center', gap: 11, background: C.card, border: `1px solid ${al.border}`, clipPath: cut(10), padding: '12px 13px', cursor: 'pointer', textAlign: 'left' }}>
            ...
            <span style={{ flex: 'none', fontFamily: PIXEL, fontSize: 11, color: al.color }}>{al.cta}</span>
          </button>
        ))}
      </div>

      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        <MicroLabel color={C.ok} size={11} tracking=".08em"><LiveDot color={C.ok} size={11} style={{ marginRight: 6 }} /> HOY EN EL CLUB</MicroLabel>
        <MicroLabel color={C.text3} size={11} tracking="normal">{ctx.hoy.length} SESIONES</MicroLabel>
      </div>
        ...
        {ctx.hoy.map((h, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 11, background: C.card, border: `1px solid ${BORDER.neutral}`, clipPath: cut(10), padding: '12px 13px' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <p style={{ margin: '1px 0 0', fontSize: 11, color: C.text3 }}>{h.sub}</p>
            </div>
            <span style={{ flex: 'none', fontFamily: PIXEL, fontSize: 11, color: h.chipColor, display: 'flex', alignItems: 'center', gap: 4 }}>
