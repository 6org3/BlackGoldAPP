  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 11, background: C.card, border: `1px solid ${r.border}`, clipPath: cut(10), padding: '11px 12px' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <p style={{ margin: '1px 0 0', fontSize: 11, color: C.text3 }}>{r.sub}</p>
      </div>
      <span style={{ flex: 'none', fontFamily: PIXEL, fontSize: 11, color: r.montoColor || C.text }}>{r.monto}</span>
      <button type="button" onClick={r.onBtn} style={{ flex: 'none', minHeight: 44, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '9px 11px', clipPath: cut(7), background: r.btnBg, border: `1px solid ${r.btnBorder}`, color: r.btnFg, fontFamily: PIXEL, fontSize: 11, letterSpacing: '.03em', cursor: 'pointer' }}>{r.btnLabel}</button>
    </div>
  );
}
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, background: C.card, border: `1px solid ${BORDER.goldMid}`, clipPath: cut(14), padding: 16, marginBottom: 14 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div>
            <MicroLabel color={C.text3} size={11} tracking=".06em">RECAUDADO</MicroLabel>
            <p style={{ margin: '3px 0 0', fontFamily: PIXEL, fontSize: 19, color: C.gold }}>{ctx.recaudadoLabel}</p>
          </div>
          <div>
            <MicroLabel color={C.text3} size={11} tracking=".06em">POR COBRAR</MicroLabel>
            <p style={{ margin: '3px 0 0', fontFamily: PIXEL, fontSize: 14, color: C.warn }}>{ctx.porCobrarLabel}</p>
          </div>
          <div>
            <MicroLabel color={C.text3} size={11} tracking=".06em">META DEL MES</MicroLabel>
      ...
      <div style={{ background: C.card, border: `1px solid ${BORDER.neutral}`, clipPath: cut(12), padding: 14, marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 7 }}>
          <MicroLabel color={C.text2} size={11} tracking=".04em">MENSUALIDADES</MicroLabel>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 7 }}>
          <MicroLabel color={C.text2} size={11} tracking=".04em">SESIONES 1V1</MicroLabel>
        ...
        <div style={{ display: 'flex', gap: 14, paddingTop: 11, borderTop: `1px solid ${BORDER.neutral06}` }}>
          <span style={{ fontFamily: PIXEL, fontSize: 11, color: C.danger }}>{ctx.vencidosLabel}</span>
          <span style={{ fontFamily: PIXEL, fontSize: 11, color: C.info }}>{ctx.becadosLabel}</span>
        </div>
      </div>

      {/* Por verificar */}
      <MicroLabel color={C.gold} size={11} style={{ margin: '0 0 8px' }}>POR VERIFICAR · {ctx.verificarCount}</MicroLabel>
      ...
      {/* Vencidos / mora */}
      <MicroLabel color={C.danger} size={11} style={{ margin: '0 0 8px' }}>VENCIDOS · MORA</MicroLabel>
