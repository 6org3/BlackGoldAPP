    ...
    <div style={{ background: C.card, border: `1px solid ${c.border}`, clipPath: cut(12), padding: '13px 14px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <p style={{ margin: '2px 0 0', fontSize: 11, color: C.text3 }}>{c.cats}</p>
        </div>
        <div style={{ textAlign: 'right', flex: 'none' }}>
          <p style={{ margin: 0, fontFamily: PIXEL, fontSize: 16, color: c.rankColor }}>{c.metricVal}</p>
          <p style={{ margin: '2px 0 0', fontFamily: PIXEL, fontSize: 11, color: C.text3 }}>{c.metricLabel}</p>
        </div>
      </div>
      <div style={{ display: 'flex', gap: 16, marginTop: 11, paddingTop: 11, borderTop: '1px solid rgba(255,255,255,.06)' }}>
        <div><p style={{ margin: 0, fontFamily: PIXEL, fontSize: 11, color: C.ok }}>{c.asist}</p><p style={{ margin: '2px 0 0', fontFamily: PIXEL, fontSize: 11, color: C.text3 }}>ASISTENCIA</p></div>
        <div><p style={{ margin: 0, fontFamily: PIXEL, fontSize: 11, color: C.text }}>{c.ses}</p><p style={{ margin: '2px 0 0', fontFamily: PIXEL, fontSize: 11, color: C.text3 }}>SESIONES</p></div>
        <div><p style={{ margin: 0, fontFamily: PIXEL, fontSize: 11, color: C.gold }}>{c.xp}</p><p style={{ margin: '2px 0 0', fontFamily: PIXEL, fontSize: 11, color: C.text3 }}>XP REPARTIDO</p></div>
      </div>
