...
export default function PantallaAtletaEventos({ ctx, embedded = false }) {
  ...
  return (
    <div>
        ...
        {ctx.eventos.map((e, i) => (
          <div key={i} style={{ background: C.card, border: `1px solid ${e.voy ? 'rgba(52,211,153,.4)' : BORDER.neutral}`, clipPath: cut(12), padding: 15 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <MicroLabel color={C.text3} size={11} tracking="normal" style={{ flex: 'none' }}>{e.confLabel}</MicroLabel>
            </div>
            <button
              type="button"
              onClick={e.onVoy}
      ...
      <MicroLabel color={C.text3} size={11} style={{ margin: '0 0 8px' }}>HISTORIAL</MicroLabel>
        ...
        {ctx.historial.map((h, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 11, background: C.card, border: `1px solid ${BORDER.neutral}`, clipPath: cut(10), padding: '11px 13px' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {h.sub && <p style={{ margin: '1px 0 0', fontSize: 11, color: C.text3 }}>{h.sub}</p>}
            </div>
