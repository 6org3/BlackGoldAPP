import { PIXEL, C, TEXT_MIN, cut as cutPath } from './arcadeTokens';
...
export default function KpiTile({ label, val, color = C.gold, sub, border = 'rgba(255,255,255,.08)', labelSize = TEXT_MIN }) {
  return (
    <div style={{ background: C.card, border: `1px solid ${border}`, clipPath: cutPath(10), padding: '13px 14px' }}>
      ...
      {sub && <p style={{ margin: '6px 0 0', fontSize: 11, color: C.text2 }}>{sub}</p>}
    </div>
  );
}
