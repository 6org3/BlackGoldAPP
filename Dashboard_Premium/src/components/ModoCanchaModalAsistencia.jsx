    ...
    <div className="space-y-6 flex flex-col flex-1 min-h-0">
      <div className="flex justify-between items-center mb-2">
        ...
        <div className="flex space-x-2">
          <button onClick={() => checkAllAsistencia(true)} className="text-xs bg-emerald-500/20 text-emerald-400 px-4 py-2.5 min-h-11 rounded-full font-bold uppercase hover:bg-emerald-500/30">Todos Presentes</button>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2 mb-2">
          <div className="relative flex-1">
            ...
            <input type="search" autoComplete="off" placeholder="Buscar nombre o cédula..."
                   ...
                   className="w-full bg-black/40 border border-white/10 rounded-lg pl-8 pr-3 py-2 text-base text-white placeholder-gray-500 focus:border-orange-400 focus:outline-none" />
          </div>
          <select value={nivelSeleccionado} onChange={(e) => setNivelSeleccionado(e.target.value)}
                  className="w-full sm:w-1/3 bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-base text-white focus:border-orange-400 focus:outline-none">
        ...
        {atletasParaSesion.length === 0 ? (
          ...
          atletasParaSesion.map(a => (
            <div key={a.atleta_id} className="flex items-center justify-between p-3 bg-white/5 border border-white/10 rounded-xl hover:border-white/30 transition-colors">
              ...
              <div className="flex gap-1 bg-[#09090b] rounded-lg p-1 border border-white/10">
                <button onClick={() => handleMarcarAsistencia(a.atleta_id, true)}
                  aria-pressed={asistencia[a.atleta_id] === true}
                  className={`px-5 py-3 min-h-11 rounded-md text-sm font-bold transition-all ${asistencia[a.atleta_id] === true ? 'bg-emerald-500 text-black shadow-[0_0_10px_rgba(16,185,129,0.3)]' : 'text-gray-500 hover:text-white'}`}>P</button>
                <button onClick={() => handleMarcarAsistencia(a.atleta_id, false)}
                  aria-pressed={asistencia[a.atleta_id] === false}
                  className={`px-5 py-3 min-h-11 rounded-md text-sm font-bold transition-all ${asistencia[a.atleta_id] === false ? 'bg-red-500 text-white shadow-[0_0_10px_rgba(239,68,68,0.3)]' : 'text-gray-500 hover:text-white'}`}>A</button>
              </div>
            </div>
          ))
        )}
