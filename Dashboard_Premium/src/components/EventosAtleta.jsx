...
const TIPO_COLORS = {
  partido:                'text-[#FFD700] border-[#FFD700]/30 bg-[#FFD700]/5',
  torneo:                 'text-purple-400 border-purple-500/30 bg-purple-500/5',
  entrenamiento_especial: 'text-emerald-400 border-emerald-500/30 bg-emerald-500/5',
  clinica:                'text-blue-400 border-blue-500/30 bg-blue-500/5',
  reunion:                'text-gray-400 border-gray-500/30 bg-gray-500/5',
  evaluacion:             'text-orange-400 border-orange-500/30 bg-orange-500/5',
  social:                 'text-pink-400 border-pink-500/30 bg-pink-500/5',
};

const RSVP_CONFIG = {
  asiste:    { label: 'Confirmaré',   icon: CheckCircle2, color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/30' },
  no_asiste: { label: 'No asistiré',  icon: XCircle,      color: 'text-red-400',     bg: 'bg-red-500/10 border-red-500/30' },
  duda:      { label: 'Tengo dudas',  icon: HelpCircle,   color: 'text-amber-400',   bg: 'bg-amber-500/10 border-amber-500/30' },
  pendiente: { label: 'Sin respuesta',icon: Clock,         color: 'text-gray-500',    bg: 'bg-white/5 border-white/10' },
};
  ...
  if (eventos.length === 0) {
    return (
      <div className="max-w-2xl mx-auto flex flex-col items-center justify-center h-48 text-center">
        ...
        <p className="text-gray-500 font-bold uppercase tracking-widest text-xs">Sin convocatorias aún</p>
...
function EventoCard({ evento, index, past, onRsvp, updating }) {
  ...
  return (
    <motion.div
      ...
      className={`bg-[#0d0d0f] border rounded-2xl p-5 ${past ? 'border-white/5 opacity-70' : 'border-white/10'}`}
    >
      {/* Header */}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1.5">
            ...
            {evento.estado === 'cancelado' && (
              <span className="text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full border text-red-400 border-red-500/30 bg-red-500/5">
                Cancelado
              </span>
            )}
          </div>
          ...
          {evento.rival && (
            <p className="text-[10px] text-gray-500 font-bold mt-0.5">vs. {evento.rival}</p>
          )}
        </div>

        {/* Fecha */}
        <div className="text-right shrink-0">
          <p className="text-[#FFD700] font-black text-xs uppercase tracking-wide">{fechaStr}</p>
          {evento.hora_inicio && (
            <p className="text-[10px] text-gray-500 font-bold mt-0.5">{evento.hora_inicio.slice(0,5)}</p>
          )}
        </div>
      </div>
      ...
      {past && evento.resultado && (
        <div className={`mb-4 px-3 py-2 rounded-xl text-center font-black uppercase tracking-widest text-xs border ${
          evento.resultado === 'ganado'  ? 'text-emerald-400 border-emerald-500/30 bg-emerald-500/10' :
          evento.resultado === 'perdido' ? 'text-red-400 border-red-500/30 bg-red-500/10' :
      ...
      {!past && evento.estado !== 'cancelado' && (
        <div className="flex flex-wrap gap-2">
          ...
          {updating && <Loader2 size={14} className="text-[#FFD700] animate-spin self-center" />}
        </div>
      )}
      ...
      {!past && evento.rsvp !== 'pendiente' && (
        <div className={`mt-2 flex items-center gap-1.5 ${rsvpCfg.color}`}>
          <RsvpIcon size={11} />
          <span className="text-[9px] font-bold">Tu respuesta: {rsvpCfg.label}</span>
        </div>
      )}
    </motion.div>
  );
