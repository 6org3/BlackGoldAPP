...
serve(async (req) => {
  ...
  // Ruta esperada: .../brain-gateway/atleta/{id}/diagnostico
  ...
  if (!atletaId || recurso !== 'diagnostico') {
    return jsonResponse({ error: 'Ruta no reconocida. Usa POST /brain-gateway/atleta/{id}/diagnostico.' }, 404);
  }
  ...
  const { data: target, error: eTarget } = await admin
    .from('atletas')
    .select('id, usuario_id, usuarios!inner!atletas_usuario_id_fkey(id, nombre, fecha_nacimiento, club, categoria_feb)')
    .eq('id', atletaId)
    .single();
  if (eTarget || !target) return jsonResponse({ error: 'Atleta no encontrado.' }, 404);

  // 4. Alcance por rol y club ANTES de leer datos analíticos.
  const rechazo = await fueraDeAlcance(admin, caller as Caller, target as unknown as Target);
  if (rechazo) return jsonResponse({ error: rechazo }, 403);

  // 5. Evaluaciones recientes (misma ventana que analyze_athlete_pillars).
  ...
  // 6. El mismo cerebro que el MCP, en JSON estructurado para las cards.
  const usuarios = (target as unknown as Target).usuarios;
  ...
  return jsonResponse({
    atleta: { id: (target as unknown as Target).id, nombre: usuarios.nombre, categoria },
    diagnostico: {
      categoria,
      subPilares,
      debilidades: debiles,
