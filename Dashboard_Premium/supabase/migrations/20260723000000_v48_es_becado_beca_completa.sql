-- ============================================================================
-- v48 — es_becado SIGNIFICA "BECA COMPLETA" (100%) EN EL MONTO, NO SOLO EN LA
--        ETIQUETA (bug heredado desde v34)
-- ============================================================================
-- Desde v34, `generar_pagos_mes` trata `es_becado = true` como beca completa
-- para el ESTADO ('Becado') y la NOTA ('Beca completa'), agrupándolo con
-- `beca_pct >= 100`. Pero el MONTO usaba `pct = GREATEST(desc_ind, beca,
-- herm_pct)`, que ignora el booleano: un atleta con `es_becado = true` y
-- `beca_pct < 100` (los dos campos son independientes) salía con
-- estado='Becado' + nota='Beca completa' pero `monto_final` COMPLETO. Un cobro
-- que contradice su propia etiqueta.
--
-- Al momento de esta migración el bug NO tiene víctimas (los 23 becados activos
-- tienen beca_pct=100), pero es LATENTE: basta marcar es_becado sin poner
-- beca_pct=100 para cobrarle a un becado. Este fix lo blinda.
--
-- FIX (una línea, en el CTE `final`): el descuento efectivo pasa a 100% cuando
-- `es_becado OR beca >= 100`. `pct` alimenta tanto la mensualidad como los
-- add-ons (v39), así que la corrección es consistente en las dos ramas sin
-- tocar nada más: monto_final del becado = 0 y `descuento_pct` guardado = 100.
--   · La media cuota de v46 se compone bien: base_mes/2 × (1−100/100) = 0.
--   · Las notas ('Beca completa') y el estado ('Becado') ya eran correctos; solo
--     el monto los desmentía. Ahora concuerdan.
--
-- Cuerpo textual de v46 (media cuota) + este delta. Firma INTACTA — ver v39
-- §4/§5: cambiar el orden de los parámetros crearía una sobrecarga silenciosa;
-- el guard de abajo revienta si quedan ≠1 definiciones.
-- ============================================================================
...
BEGIN
  ...
  final AS (
    SELECT calc.*,
           -- v48: es_becado es "beca completa" (100%), igual que beca>=100.
           -- Sin este CASE, el GREATEST ignoraba el booleano y un es_becado con
           -- beca_pct<100 salía estado='Becado'/nota='Beca completa' pero con
           -- monto_final completo. `pct` fluye a mensualidad Y add-ons.
           CASE WHEN es_becado OR beca >= 100 THEN 100
                ELSE GREATEST(desc_ind, beca, herm_pct) END AS pct,
...
-- Guard de v39 §5 (repetido aquí como en v40/v42/v46): que nunca convivan dos
