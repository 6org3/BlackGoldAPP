...
- **H1-D1** — ✅ **CERRADO (2026-07-29).** Verificación por tabla en [`docs/verificacion_rls_h1_d1.md`](verificacion_rls_h1_d1.md). La contradicción se resuelve **a favor de v24**: `supabase migration list` da **59/59 en paridad** (producción es exactamente la suma de las migraciones del repo) y ninguna de las 4 tablas de v18 conserva `FOR ALL USING (true)` — v24 las borró en bloque y las recreó. Las 6 tablas del alcance tienen RLS activa, policies `TO authenticated` (anon no las alcanza) y aislamiento por club vía `club_de_atleta()`/`club_de_pago()`; la única excepción es la lectura del catálogo `misiones`, global a propósito. La verificación destapó además que la suite RLS no tenía **ningún** assert sobre las tres tablas que el agente escribiría: se añadieron 12 y la corrida completa contra producción da **124/124 en verde**. El bloqueo técnico queda levantado — F1/F2/F3 siguen pendientes de decisiones del owner, que es otra cosa. *(Enunciado original, para contexto:)* `pendientes_post_beta.md` #1 (2026-07-04) marcaba RLS como P0 sin resolver, pero `blackgold-mcp/src/index.js` y `pagos_diseno.md` §7.2 asumían "RLS v24" vigente.
