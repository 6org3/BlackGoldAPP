...
5. ✅ **Aplicada en producción (2026-07-07)** con `npx supabase db push`. Notas de la aplicación:
   - El primer intento falló (rollback limpio) por `uuid = uuid[]`: el patrón `x = ANY ((select fn_array()))` se parsea como sub-link fila a fila, no como array — corregido globalmente (28 sitios) a `x IN (SELECT unnest(fn_array()))`, que además garantiza initplan (una evaluación por consulta).
   - El historial remoto tenía una migración huérfana `20260705224803` (aplicada a mano el 2026-07-05 22:48, sin archivo en el repo, registrada temporalmente con un stub `SELECT 1`). **Reconciliada (2026-07-09):** el archivo real (`20260705224803_v24_misiones_pilar_resistencia_recuperacion.sql` — CHECK de `misiones_pilar_check` ampliado a `resistencia`/`recuperacion`) vivía sin fusionar en una rama aparte; se recuperó y reemplazó al stub, así que el repo ya versiona el SQL real de esa versión.
   - `SUPABASE_SERVICE_ROLE_KEY` agregada al `.env` local del MCP (key formato nuevo `sb_secret_…`, copiada de `Dashboard_Premium/.env.local`; nunca en el repo).
   - Smoke técnico post-aplicación: anon → `42501 permission denied` en `usuarios`/`atletas`/`atleta_readiness` (REST crudo y supabase-js); MCP con service key → OK (856 usuarios visibles); `resolver_email_login` responde 200 a anon; `registrar_publico` existe y valida entradas (P0001 esperado con payload vacío).
6. ⬜ **URGENTE — Usuario:** desplegar el frontend a Vercel YA: el formulario de registro público **en producción está caído desde la aplicación de v24** (el código desplegado aún inserta como anon → 42501) hasta que se despliegue el `registroPublicoService.js` nuevo. Login y toda la operación autenticada funcionan normal.
7. ✅ **Validación por rol completada (2026-07-07)** con `scripts/validar_rls_por_rol.js` — suite reutilizable que crea cuentas QA aisladas (`QA_RLS_*`), ejecuta 25 asserts positivos y negativos con sesiones de Auth reales contra la base de producción y se limpia sola: **25/25 en verde** (anon bloqueado; atleta no ve a otros ni infla XP ni se auto-promueve; padre ve exactamente a su hijo; coach opera todo, crea plantillas —roto pre-v24— y no puede crear superadmins; registro público end-to-end con login del recién registrado). Re-correrla tras cualquier cambio de políticas.
8. ✅ **Hallazgo y fix del registro público (independiente de v24):** GoTrue rechaza los emails sintéticos (`cedula@sinacceso…internal`) en el `signUp` PÚBLICO (valida el dominio) aunque la Admin API sí los acepta — es decir, el registro de atletas sin correo real estaba roto **desde la Fase 1** (latente: nadie se registró desde entonces). Fix: Edge Function `registro-publico` (service_role, patrón `generar-misiones-ia`) que ejecuta la RPC y crea las cuentas con `admin.createUser`; `registroPublicoService.js` quedó reducido a un `functions.invoke`, y el navegador del registrante ya no queda logueado como el último `signUp`. Desplegada con `npx supabase functions deploy registro-publico` (aprobado por el usuario).
