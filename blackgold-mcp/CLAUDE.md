...
### Inventario actual (22 docs, ~425 fragmentos; `npm run rack` da el detalle vivo)

- **`metodologia`** (en `knowledge/`): `fundamentos_iniciacion_vinueza` (guía raíz ecuatoriana, Vinueza), `taxonomia_pilares_subpilares` (ontología del sistema), `fases_sensibles_entrenabilidad`, `periodizacion_entrenamiento_anual`, `crecimiento_maduracion` (edad biológica/PHV), `deteccion_talentos`, `nutricion_adolescente`, `antropometria_composicion_corporal` (protocolo de medición, IMC pediátrico, comunicación), `recuperacion_carga_descanso`, `prevencion_lesiones_baloncesto`, `trabajo_casa_atleta`, `perfil_entrenador`, `sesgo_muestra_pequena` (regla n<3: cuándo un diagnóstico es firme vs. provisional).
- **`mentalidad`** (en `knowledge/`): `clima_motivacional_abandono` (clima de maestría, presión parental, burnout y prevención del abandono).
