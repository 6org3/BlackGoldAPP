...
- `Dashboard_Premium/supabase/migrations/` — migraciones SQL versionadas con la convención de Supabase CLI (`<timestamp>_descripcion.sql`), consolidadas ahí desde julio 2026 (antes vivían sueltas en la raíz del repo y de `Dashboard_Premium/`). 58 archivos a la fecha; última con número de versión: **v52** (`20260729120000_v52_endurecimiento_pre_produccion.sql` — cinco huecos de RLS de la auditoría pre-producción: `beca_pct` protegida, `es_owner_activo()`, pagos solo-dueño, `resolver_email_login` sin oráculo de existencia y la tabla `registro_intentos`. **Escrita, NO aplicada todavía** — requiere `npx supabase db push`), precedida por **v51** (`20260726120000_v51_gastos_contabilidad_gestion.sql`, tabla `gastos` para `blackgold-negocio-mcp` — aplicada a producción el 2026-07-27) y por **v50** (`20260723120000_v50_padre_ve_sesiones_grupales.sql`), con un parche sin número entre estas dos últimas (`revierte_plantilla_id_duplicado.sql`, revierte un diseño descartado de un PR paralelo). Ver detalle más abajo.
...
Deuda P2 identificada y aún no abordada: `registro-publico` sin captcha ni rate limit (la tabla `registro_intentos` ya existe en v52, falta cablearla) y sin rollback si `admin.createUser` falla tras la RPC — deja la cédula ocupada para siempre; el grant directo a `anon` sobre funciones que `REVOKE ... FROM PUBLIC` no quita (`club_de_usuario`, `club_de_atleta`); CORS `*` en las 6 Edge Functions; y `copiloto` sin cuota por usuario.
