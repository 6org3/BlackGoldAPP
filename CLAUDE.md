...
**Paridad local↔producción: verificada el 2026-07-29** con `npx supabase migration list` — **las 59 migraciones del repo están aplicadas en producción, sin ninguna pendiente.** La duda que este documento dejaba abierta al respecto queda cerrada.
...
- **Gate de cuota del copiloto** en la Edge Function. v53 ya trae la tabla `copiloto_uso` y la función `consumir_cuota_copiloto`; falta invocarla desde `copiloto/index.ts`, que hoy no tiene ningún límite de frecuencia — un atleta autenticado puede quemar la `ANTHROPIC_API_KEY` en bucle.
...
- **Recuperación de contraseña sin pasar por el club** (entrega 3 del arranque con datos reales): hoy no hay "olvidé mi contraseña" porque casi ninguna cuenta tiene correo real — el email de Auth suele ser el sintético `<cédula>@sinacceso.blackgoldapp.internal`, que no llega a nadie. Hace falta correo obligatorio del representante, `resetPasswordForEmail`, SMTP configurado (acción del dueño) y sincronizar `auth.users.email` cuando cambia `usuarios.correo`. Mientras tanto la única vía es que el dueño regenere el acceso desde `/admin/atletas`.
- **Una familia no puede compartir teléfono ni correo** (entrega 4): ambos son UNIQUE en `usuarios`, así que el segundo hermano con los mismos datos de contacto falla — y el error que se ve es el engañoso "cédula ya registrada".
...
---
Última actualización: 2026-07-30 (PR #158: la contraseña inicial deja de ser la cédula y se exige cambiarla en el primer ingreso; séptima Edge Function, `cambiar-password`, desplegada).
