...
- Las migraciones de base de datos son aditivas (`ALTER TABLE ... IF NOT EXISTS`). Agregar un nuevo archivo en `Dashboard_Premium/supabase/migrations/` con timestamp nuevo (`npx supabase migration new <descripcion>` genera el nombre correcto) en lugar de editar uno ya aplicado. Última: v25 (purga de `usuarios.contrasena_hash`; v24 dejó RLS real por `auth.uid()` con anon sin acceso directo a tablas y el registro público server-side — Edge Function `registro-publico` + RPC `registrar_publico()`. Ver `docs/plan_remediacion_seguridad.md` Fase 2 y validar con `node scripts/validar_rls_por_rol.js` tras cualquier cambio de políticas). Aplicar con `npx supabase db push` (proyecto ya vinculado, ver `supabase/.temp/project-ref`) en vez de pegar el SQL a mano en el editor web — así el historial de migraciones aplicadas queda registrado server-side.
